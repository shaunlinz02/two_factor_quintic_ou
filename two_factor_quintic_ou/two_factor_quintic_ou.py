...
class TwoFactorQuinticOUModel:
    ...
    def __init__(self, params, fv_curves):
        ...
        self.xi_nodes = fv_curves.get('xi_nodes')
    ...
    def show_fwd_var(self):
        ...
        print(f"  xi_nodes: {self.xi_nodes}")
    ...
    def fv_curve_generator(self, t):
        """
        Method to generate the forward variance curve using xi_nodes and T_nodes.
        """
        # total variance swaps
        var_swap_T = np.cumsum(self.xi_nodes * np.diff(np.concatenate(([0], self.T_nodes))))
        
        # Create first spline: Interpolate total variance swap curve
        inter_spline_var_swap_T = interpolate.splrep(self.T_nodes, var_swap_T, k=3)
        
        # Adjust initial value and create second spline for smoother transition at t=0
        inter_spline_var_swap_T = interpolate.splrep(
            np.concatenate(([0.0001], self.T_nodes)),
            np.concatenate((self.xi_nodes[0:1] * 0.0001, var_swap_T)),
            k=3
        )
        
        # Evaluate derivative of the curve at t (forward curve)
        fv_curve = interpolate.splev(t, inter_spline_var_swap_T, der=1)
    ...
    def vix_pricing_quantization(self, T, strike_perc_vix, compute_iv=True):
        ...
        """
        This method performs VIX pricing using the two-factor quintic OU model
        """
        ...
        if not hasattr(self, 'tt_gs'):
            ...
            tt_gs, w_t_gs = self.tt_gs, self.w_t_gs

        w_tt, tt = self.w_t_gs / 2 * (T_delta - T), 0.5 * (self.tt_gs + 1) * (T_delta - T) + T

        # Compute g_0 squared
        FV_curve_all_vix = self.fv_curve_generator(tt)
        FV_curve_all_vix = np.sqrt(FV_curve_all_vix**2)
        FV_curve_all_vix[tt<self.T_nodes[0]] = self.xi_nodes[0]    #### xi_0(t) for t smaller than the grid this the first value of fvc
    ...
    def spx_pricing_mc(self,x1,x2,S_0,T,strikes,n_steps,N_sims,w1,compute_iv = True):
        ...
        fv_curve = self.fv_curve_generator(tt).reshape(-1, 1)
        fv_curve = np.sqrt(fv_curve**2)
        fv_curve[:,0][tt<self.T_nodes[0]] = self.xi_nodes[0]    #### xi_0(t) for t smaller than the grid this the first value of fvc
    ...
    def spx_ssr_mc(self,T,n_steps,N_sims,w1):
        ...
        ssr_h = 0.0001
        rho_ssr = self.rho
        v_0 = self.xi_nodes[0] ### assumes x_i(t) is constant before the its first node
